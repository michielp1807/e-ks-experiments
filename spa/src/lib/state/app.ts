...
import type { AppData, AuthorisedAgent, CandidateList, ListSubmitter, Person, PoliticalGroup } from '$lib/types';
...
function createAppState() {
	...
	return {
		...
		updatePoliticalGroup(politicalGroup: PoliticalGroup) {
			...
			store.update((state) => ({ ...state, politicalGroup: normalizedPoliticalGroup }));
		},
		upsertPerson(person: Person) {
			...
			store.update((state) => {
				const persons = state.persons.filter((entry) => entry.id !== normalizedPerson.id);
				return {
					...state,
		...
		deletePerson(personId: string) {
			store.update((state) => ({
				...
				candidateLists: state.candidateLists.map((list) => ({
					...list,
					candidateIds: list.candidateIds.filter((candidateId) => candidateId !== personId)
				}))
			}));
		},
		upsertAuthorisedAgent(agent: AuthorisedAgent) {
			...
			store.update((state) => ({
				...state,
				authorisedAgents: [
					...state.authorisedAgents.filter((entry) => entry.id !== normalizedAgent.id),
					normalizedAgent
		...
		deleteAuthorisedAgent(agentId: string) {
			store.update((state) => ({
				...state,
				authorisedAgents: state.authorisedAgents.filter((entry) => entry.id !== agentId)
			}));
		},
			...
			store.update((state) => {
				if (substitute) {
					return {
						...state,
						substituteListSubmitters: [
							...state.substituteListSubmitters.filter((entry) => entry.id !== normalizedSubmitter.id),
							normalizedSubmitter
						].sort((a, b) => a.name.lastName.localeCompare(b.name.lastName, 'nl'))
					};
				}
				...
				return {
					...state,
					listSubmitters: [
						...state.listSubmitters.filter((entry) => entry.id !== normalizedSubmitter.id),
						normalizedSubmitter
					].sort((a, b) => a.name.lastName.localeCompare(b.name.lastName, 'nl'))
				};
			});
		...
		deleteListSubmitter(submitterId: string, substitute = false) {
			store.update((state) => {
				const next: AppData = {
					...
					candidateLists: state.candidateLists.map((list) => ({
						...list,
						listSubmitterId: list.listSubmitterId === submitterId ? null : list.listSubmitterId,
						substituteListSubmitterIds: list.substituteListSubmitterIds.filter((id) => id !== submitterId)
					}))
				};
				...
				if (substitute) {
					...
					next.listSubmitters = state.listSubmitters.filter((entry) => entry.id !== submitterId);
				}
		...
		upsertCandidateList(candidateList: CandidateList) {
			const normalizedCandidateList: CandidateList = {
				...
				substituteListSubmitterIds: [...candidateList.substituteListSubmitterIds]
			};

			store.update((state) => ({
				...state,
				candidateLists: [
					...state.candidateLists.filter((entry) => entry.id !== normalizedCandidateList.id),
		...
		deleteCandidateList(candidateListId: string) {
			store.update((state) => ({
				...state,
				candidateLists: state.candidateLists.filter((list) => list.id !== candidateListId)
			}));
		}
	};
}
