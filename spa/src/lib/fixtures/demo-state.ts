...
function buildPersons() {
	return loadFixturePersons().map((record, index): Person => ({
		...
		personalData: {
			gender: record.geslacht === 'M' ? 'male' : record.geslacht === 'V' ? 'female' : '',
...
export function createDemoState(): AppData {
	...
	const firstListPersonIds = persons.slice(0, FIXTURE_CANDIDATE_LIST_SIZE).map((person) => person.id);
	...
	return {
		...
		candidateLists: [
			...
			},
			{
				...
				candidateIds: validPersonIds.slice(0, Math.floor(FIXTURE_CANDIDATE_LIST_SIZE / 2)),
				listSubmitterId: FIXTURE_IDS.listSubmitter,
				substituteListSubmitterIds: [FIXTURE_IDS.substituteSubmitter1],
