...
export function emptyDutchAddress() {
	return { postalCode: '', houseNumber: '', houseNumberAddition: '', streetName: '', locality: '' };
}
...
export function formatName(name: Name) {
	return [name.initials, name.firstName, name.lastNamePrefix, name.lastName].filter(Boolean).join(' ').trim();
}
...
export function addressLine1(address: InternationalAddress) {
	return [address.streetName, `${address.houseNumber}${address.houseNumberAddition}`.trim()]
		.filter(Boolean)
		.join(' ')
		.trim();
}
...
export function addressLine2(address: InternationalAddress) {
	const country = address.country && address.country !== 'NL' ? ` (${address.country})` : '';
	return [address.postalCode, address.locality].filter(Boolean).join(' ').trim() + country;
}
...
export function isPoliticalGroupComplete(data: AppData) {
	const group = data.politicalGroup;
	return group.longListAllowed !== null && Boolean(group.legalName.trim() && group.displayName.trim());
}
...
export function duplicateDistricts(data: AppData, candidateListId: string) {
	...
	return (
		data.candidateLists
			...
			?.electoralDistricts.filter((district) => (counts.get(district) ?? 0) > 1) ?? []
	);
}
...
export function candidateListCanSubmit(data: AppData, candidateList: CandidateList) {
...
export function districtSummary(codes: DistrictCode[], locale: Locale) {
	if (codes.length === DISTRICTS.length) {
		return locale === 'nl' ? 'Alle kieskringen' : 'All electoral districts';
	}

	return codes.map((code) => districtTitle(code, locale)).join(', ');
}
