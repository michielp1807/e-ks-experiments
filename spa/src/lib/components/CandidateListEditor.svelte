<script lang="ts">
	import { availableDistricts, candidateListCanSubmit, districtSummary, duplicateDistricts, formatName } from '$lib/helpers';
	...
	const currentAvailableDistricts = $derived(availableDistricts(state, candidateList.id));
	...
	function toggleDistrict(code: DistrictCode) {
		if (candidateList.electoralDistricts.includes(code)) {
			candidateList.electoralDistricts = candidateList.electoralDistricts.filter((entry) => entry !== code);
			return;
		}

		candidateList.electoralDistricts = [...candidateList.electoralDistricts, code];
	}

	function toggleCandidate(personId: string) {
		if (candidateList.candidateIds.includes(personId)) {
			candidateList.candidateIds = candidateList.candidateIds.filter((entry) => entry !== personId);
			return;
		}
...
<form class="form stack-lg" onsubmit={(event) => { event.preventDefault(); onSubmit?.(candidateList); }}>
	<fieldset>
			...
			{#each DISTRICTS as district}
				...
				>
					<input
						...
						disabled={!currentAvailableDistricts.some((entry) => entry.code === district.code) && !candidateList.electoralDistricts.includes(district.code)}
						id={`district-${district.code}`}
						onchange={() => toggleDistrict(district.code)}
					/>
					<label for={`district-${district.code}`}>{district.title[state.locale]}</label>
				</div>
			{/each}
		</div>
		{#if candidateList.electoralDistricts.length > 0}
			<p class="hint mt-md">{districtSummary(candidateList.electoralDistricts, state.locale)}</p>
		{/if}
		{#if duplicateCodes.length > 0}
			<p class="note-warning mt-md">Deze lijst deelt kieskringen met een andere lijst.</p>
		{/if}
	</fieldset>

	<fieldset>
		<legend><h2>Lijstindieners</h2></legend>
			...
			<div class="checklist">
				{#each state.substituteListSubmitters as submitter}
					<label class="checkbox radio-card">
						<input
							type="checkbox"
							checked={candidateList.substituteListSubmitterIds.includes(submitter.id)}
							onchange={() => {
								if (candidateList.substituteListSubmitterIds.includes(submitter.id)) {
									candidateList.substituteListSubmitterIds = candidateList.substituteListSubmitterIds.filter(
										(id) => id !== submitter.id
									);
								} else {
					...
					{#each candidates as person, index}
						<tr>
							...
							<td class="table-actions">
								<button class="button secondary sm" type="button" onclick={() => moveCandidate(index, -1)}>
									Omhoog
								</button>
								<button class="button secondary sm" type="button" onclick={() => moveCandidate(index, 1)}>
									Omlaag
	...
	<div class="buttons">
		...
		<span class={`status-pill ${candidateListCanSubmit(state, candidateList) ? 'complete' : 'warning'}`}>
			{candidateListCanSubmit(state, candidateList) ? 'Klaar voor vervolgstap' : 'Nog niet compleet'}
		</span>
	</div>
</form>
