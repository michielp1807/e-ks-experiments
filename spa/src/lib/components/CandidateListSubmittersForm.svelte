...
>
  <fieldset>
    <legend><h2>Lijstinleveraar</h2></legend>
            ...
            <input
              ...
              onchange={() => {
                if (draft.substituteListSubmitterIds.includes(submitter.id)) {
                  draft.substituteListSubmitterIds =
                    draft.substituteListSubmitterIds.filter(
                      (id) => id !== submitter.id,
                    );
                } else {
                  draft.substituteListSubmitterIds = [
  ...
  <div class="buttons">
    ...
    <a
      ...
      href={resolve("/candidate-lists/[id]/details", { id: draft.id })}
      >Vorige</a
    >
  </div>
</form>
