...
let internalShuffle: () => void;
function setInternalShuffle(fn: () => void) {
  internalShuffle = fn;
}
...
{:else}
  <table
    id="candidate-table"
    data-max={maxCandidates(appData.politicalGroup.longListAllowed)}
  >
    <thead>
      <tr>
        ...
        <th scope="col"
          >{candidates.length}/{maxCandidates(
            appData.politicalGroup.longListAllowed,
          )} kandidaten</th
        >
      </tr>
    </thead>
    <tbody use:sortableTable={{ onChange: onReorder, setInternalShuffle }}>
      {#each candidates as person, index (person.id)}
        <tr
          ...
          onclick={(event) => {
            ...
            goto(
              resolve("/candidate-lists/[id]/candidates/[personId]", {
                id: candidateList.id,
                personId: person.id,
              }),
            );
        ...
        >
          ...
          <td>
            ...
            {person.name.initials}{person.name.firstName
          ...
          <td>
            <a
              href={resolve("/candidate-lists/[id]/candidates/[personId]", {
                id: candidateList.id,
                personId: person.id,
              })}
              aria-label="Bewerk kandidaat"
            >
              <span>Bewerk</span>
            </a>
          </td>
