class DragSession {
	...
	setGeometry(rows: HTMLTableRowElement[]) {
		...
		this.boundaryBottom = Math.max(...rects.map((rect) => rect.bottom + scrollY));
	}
	...
	shiftOtherRows(rows: HTMLTableRowElement[], currentTop: number, currentBottom: number) {
	...
	updateLiveOrder(rows: HTMLTableRowElement[], movedAbove: Set<number>, movedBelow: Set<number>) {
		const dragIndex = this.startIndex - movedAbove.size + movedBelow.size;
		const newOrder: Array<HTMLTableRowElement | null> = new Array(rows.length).fill(null);
		...
		const compactOrder = newOrder.filter((row): row is HTMLTableRowElement => row !== null);
...
class SortableTable {
	...
	private positionCellCache = new Map<HTMLTableRowElement, HTMLTableCellElement>();
	private drag: DragSession | null = null;
	private dragStartOrder: string[] | null = null;
	private timeout: number | null = null;
	private onChange?: (order: string[]) => void;

	private mouseMove = (event: MouseEvent) => this.handlePointerMove(event.clientY);
	...
	constructor(tbody: HTMLTableSectionElement, onChange?: (order: string[]) => void) {
		this.tbody = tbody;
	...
	attach() {
		...
		globalThis.addEventListener('touchmove', this.touchMove, { passive: false });
	...
	private refreshHandles() {
		this.rows = Array.from(this.tbody.querySelectorAll<HTMLTableRowElement>('tr'));
		this.handles = Array.from(this.tbody.querySelectorAll<HTMLElement>('tr td.drag-handle'));
	}
	...
	private attachHandleEvents() {
		this.handles.forEach((handle) => {
			...
			handle.ontouchstart = (event) => {
				...
				event.preventDefault();
				this.reset();
				this.startDrag(row as HTMLTableRowElement, touch.clientY, touch.identifier);
			};
		});
	}

	private startDrag(row: HTMLTableRowElement, clientY: number, touchId: number | null = null) {
		this.refreshHandles();
		this.dragStartOrder = this.getRowOrderIds(this.rows);
		this.drag = new DragSession(row, clientY, this.rows.indexOf(row), this.rows.slice(), touchId);
	...
	private runDragUpdate() {
		...
		const { deltaY, currentTop, currentBottom } = drag.computeDragDelta(pendingY);
		drag.target.style.transform = `translate(0, ${deltaY}px)`;
		const { movedAbove, movedBelow } = drag.shiftOtherRows(this.rows, currentTop, currentBottom);
		const compactOrder = drag.updateLiveOrder(this.rows, movedAbove, movedBelow);
	...
	private autoScrollIfNeeded(pointerY: number) {
		...
		if (pointerY < threshold) {
			...
			globalThis.scrollBy(0, Math.min(step, pointerY - (viewportHeight - threshold)));
			scrolled = true;
		}
	...
	private finishDrag() {
		...
		if (draggedRow) {
			...
			this.timeout = globalThis.setTimeout(() => {
				document.querySelectorAll('.flash-success, .pos-up, .pos-down').forEach((element) => {
	...
	private reset() {
		...
		document.querySelectorAll('.flash-success, .pos-up, .pos-down, .fade-out').forEach((element) => {
			element.classList.remove('flash-success', 'pos-up', 'pos-down', 'fade-out');
		});
	}
