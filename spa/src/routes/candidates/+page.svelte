<script lang="ts">
	...
	const persons = $derived(
		appData.persons.filter((person) => {
			const haystack = `${formatName(person.name)} ${person.personalData.placeOfResidence}`.toLowerCase();
			return haystack.includes(search.toLowerCase());
		})
	);
</script>

<PageHeader title="Kandidaten" backHref="/candidate-lists" backLabel="Terug naar lijsten" />
...
<nav class="sticky-nav form">
	<div class="sticky-nav-bar toolbar">
		...
		<div class="buttons">
			<a href={resolve('/candidates/new')} class="button secondary icon-plus">Kandidaat toevoegen</a>
		</div>
	</div>
</nav>
			...
			<tbody>
				{#each persons as person}
					<tr class:warning={!isPersonComplete(person)}>
						...
						<td>{person.personalData.gender === 'male' ? 'Man' : person.personalData.gender === 'female' ? 'Vrouw' : ''}</td>
						<td>{person.personalData.placeOfResidence}</td>
						<td>{new Date(person.updatedAt).toLocaleDateString('nl-NL')}</td>
						<td><a href={resolve('/candidates/[id]', { id: person.id })}>Bewerk</a></td>
					</tr>
				{/each}
			</tbody>
		</table>
